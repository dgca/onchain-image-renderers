...
contract BitmapRendererV1 is IBitmapRendererV1 {
  ...
  function create8bitBMPData(
    ...
    _storeUint32(fileBuffer, 18, width);
    _storeUint32(fileBuffer, 22, height);
    ...
    _storeUint32(fileBuffer, 46, uint32(palette.length)); // Number of colors in palette
    _storeUint32(fileBuffer, 50, uint32(palette.length)); // Important colors

    // Write palette data
    assembly {
      let bufferDataStart := add(fileBuffer, add(0x20, 54)) // Starting index for color data in fileBuffer
      let paletteDataStart := add(palette, 0x20) // Pointer to the start of the palette data segment
      let paletteLength := mload(palette) // Load the length of the palette
    ...
    assembly {
      let pixelDataStart := add(add(fileBuffer, 0x20), add(54, paletteSize)) // Start of pixel data
      ...
      for {
        ...
        let invertedY := sub(height, add(y, 1))
        ...
        for {
          ...
          let pixelDataIndex := add(mul(invertedY, width), x)
          mstore8(
            add(bufferDataStart, x),
            mload(add(add(data, 0x20), mul(pixelDataIndex, 0x20)))
          )
        }
    ...
    assembly {
      ...
      for {
        ...
        for {
          ...
          let dataIndex := add(mul(invertedY, width), x)
          let pixelIndex := add(
            pixelDataStartIndex,
        ...
        for {
          ...
          mstore8(
            add(bufferStart, add(pixelDataStartIndex, add(mul(y, rowSize), p))),
            0
          )
        }
      }
  ...
  function _storeUint32(
    ...
    assembly {
      ...
      let ptr := add(buffer, add(index, 0x20))
